...
int main(){
    ...
    while(!glfwWindowShouldClose(window)){ // one iteration of this loop = 1 frame
        ...
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
        // this will configure any further drawing calls to draw individual triangles rather than filled solid shape
        // so it's setting the state of the state machine in which the drawing is done
