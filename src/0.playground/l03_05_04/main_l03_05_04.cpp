...
int main(){
    ...
    Shader cubeShader("cube_l03_05_04.vs", "cube_l03_05_04.fs");
    cubeShader.use();
    ...
    // cutoff(angle bw light n spot directions) determines the radius of the cone
    // for cut off we pass a cosine not an angle coz in frag shader we get cosine bw light n spot directions.
    // it's just easier to compare the 2 to decide if the frag portion is in the cone or not,
    // rather than doing conversions and comparing.
    cubeShader.setFloat("lamp.cosInnerCutOff",   glm::cos(glm::radians(12.5f)));
    cubeShader.setFloat("lamp.cosOuterCutOff",   glm::cos(glm::radians(17.5f)));
    ...
    // lamp.constant should never be < 1.0, if it goes < 1, then light intensity will increase with distance
    cubeShader.setFloat("lamp.constant",  1.0f);
    cubeShader.setFloat("lamp.linear",    0.09f);
    cubeShader.setFloat("lamp.quadratic", 0.032f);

    Shader lampShader("lamp_l03_05_04.vs", "lamp_l03_05_04.fs");
    lampShader.use();
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        cubeShader.setVec3("lamp.position",  camera.Position);
        cubeShader.setVec3("lamp.direction", camera.Front);
